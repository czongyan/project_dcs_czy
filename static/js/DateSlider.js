 (function ($) {
 	...
 	$.widget("ui.ruler", {
 		...
 		, _createTick: function (start, end, scaleOptions) {
 			var yearStr = scaleOptions.formatYear(start); //获取年份
 			var container = $("<div class='ui-ruler-tick' style='display:inline-block'><span style='position:absolute;top:-20px;'>" + (yearStr ? yearStr + '年' : '') + "</span></div>")
 ...
 (function ($, undefined) {
 	...
 	$.widget("ui.rangeSliderMouseTouch", $.ui.mouse, {
 		...
 		, _mouseDestroy: function () {
 			$(document).unbind('touchmove.' + this.widgetName, this._touchMoveDelegate).unbind('touchend.' + this.widgetName, this._touchEndDelegate);
 		...
 		, _touchStart: function (event) {
 			...
 			if (downEvent !== this._mouseDownEvent) {
 				...
 				$(document).bind('touchmove.' + this.widgetName, this._touchMoveDelegate).bind('touchend.' + this.widgetName, this._touchEndDelegate);
 			}
 		}
 	...
 	$.widget("ui.rangeSliderDraggable", $.ui.rangeSliderMouseTouch, {
 		...
 		_constraintPosition: function (position) {
 			if (this.element.parent().length !== 0 && this.cache.parent.offset !== null) {
 				position = Math.min(position, this.cache.parent.offset.left + this.cache.parent.width - this.cache.width.outer);
 ...
 (function ($, undefined) {
 	...
 	$.widget("ui.rangeSliderBar", $.ui.rangeSliderDraggable, {
 		...
 		, _create: function () {
 			...
 			this.options.leftHandle.bind("initialize", $.proxy(this._onInitialized, this)).bind("mousestart", $.proxy(this._cache, this)).bind("stop", $.proxy(this._onHandleStop, this));
 			this.options.rightHandle.bind("initialize", $.proxy(this._onInitialized, this)).bind("mousestart", $.proxy(this._cache, this)).bind("stop", $.proxy(this._onHandleStop, this));
 			this._bindHandles();
 		...
 		, _positionBar: function () {
 			...
 			this.element.css("width", width).offset({
 		...
 		, _bindHandles: function () {
 			this.options.leftHandle.unbind(".bar").bind("sliderDrag.bar update.bar moving.bar", $.proxy(this._onDragLeftHandle, this));
 			this.options.rightHandle.unbind(".bar").bind("sliderDrag.bar update.bar moving.bar", $.proxy(this._onDragRightHandle, this));
 		}
 ...
 (function ($, undefined) {
 	...
 	$.widget("ui.rangeSliderLabel", $.ui.rangeSliderMouseTouch, {
 		...
 		, _create: function () {
 			...
 			this._toggleClass();
 			this.options.handle.bind("moving.label", $.proxy(this._onMoving, this)).bind("update.label", $.proxy(this._onUpdate, this)).bind("switch.label", $.proxy(this._onSwitch, this));
 		...
 		, _createElements: function () {
 			this._valueContainer = $("<div class='ui-rangeSlider-label-value' />").appendTo(this.element);
 			this._innerElement = $("<div class='ui-rangeSlider-label-inner' />").appendTo(this.element);
 		}
 		, _handle: function () {
 ...
 (function ($, undefined) {
 	...
 	$.widget("ui.rangeSlider", {
 		...
 		, _createElements: function () {
 			...
 			this.container = $("<div class='ui-rangeSlider-container' />").css("position", "absolute").appendTo(this.element);
 			this.innerBar = $("<div class='ui-rangeSlider-innerBar' />").css("position", "absolute").css("top", 0).css("left", 0);
 			this._createHandles();
 			this._createBar();
 		...
 		, _createHandle: function (options) {
 			return $("<div />")[this._handleType()](options).bind("sliderDrag", $.proxy(this._changing, this)).bind("stop", $.proxy(this._changed, this));
 		}
 		...
 		, _createBar: function () {
 			this.bar = $("<div />").prependTo(this.container).bind("sliderDrag scroll zoom", $.proxy(this._changing, this)).bind("stop", $.proxy(this._changed, this));
 			this._bar({
 		...
 		, _createArrow: function (whichOne) {
 			var arrow = $("<div class='ui-rangeSlider-arrow' />").append("<div class='ui-rangeSlider-arrow-inner' />").addClass("ui-rangeSlider-" + whichOne + "Arrow").css("position", "absolute").css(whichOne, 0).appendTo(this.element)
 				, target;
 			if (whichOne === "right") {
 		...
 		_createLabel: function (label, handle) {
 			...
 			if (label === null) {
 				...
 				label = $("<div />").appendTo(this.element)[this._labelType()](params);
 			}
 			else {
 ...
 (function ($, undefined) {
 	...
 	$.widget("ui.dateRangeSliderHandle", $.ui.rangeSliderHandle, {
 		...
 		, _addThing: function (date, thing, factor, base) {
 			...
 			date["set" + thing](date["get" + thing]() + factor * (base || 0));
 			return date;
 		}
 		, _round: function (value) {
