...
(function (global) {
    ...
    function line_fineSegGrit(data) {
        ...
        for (var i in settings.parameterName) {
            ...
            //                            tempAreaColor[1] = settings.color_1(d[settings.selectMap]);
            //                            return y(d[settings.parameterName[i]]);
            //                        } else
            //                            return settings.strokeWidth * settings.g_dis[0];
            //                    } else { //展示MCR
            //                        if ((d.MW >= settings.mwDomain[0]) && (d.MW < settings.mwDomain[1])) {
            //                            tempAreaColor[0] = settings.color_1(d[settings.selectMap]);
            //                            return y(d[settings.parameterName[i]]);
            //                        } else
            //                            return settings.strokeWidth * settings.g_dis[0];
            //                    }
            ...
            //                    function (d) {
            //                        if (settings.segment) {
            //                            return tempAreaColor[1];
            //                        } else {
            //                            return tempAreaColor[0];
            //                        }
            //                    }
            //                ).style("fill-opacity", 1 / (settings.parameterName.length - i) * 0.8);

            if (i == 0) {
                //分层次展示
                var tempAreaColor = settings.color_1((settings.mwDomain[0] + settings.mwDomain[1]) / 2);
                if (settings.segment) { //展示MCR-SEG层
                    //ＭＣＲ　层
                    var area = d3.svg.area()
                        .interpolate("basis")
                        .x(function (d) {
                    ...
                    rect_g.append("path")
                        .attr("class", "area")
                        .attr("d", area)
                        .style("fill", tempAreaColor).style("fill-opacity", 0.4);
                    rect_g.append("path")
                        .attr("class", "area")
                        .attr("d", area1)
                        .style("fill", tempAreaColor).style("fill-opacity", 0.8);
                } else { //展示MCR层
                    var area = d3.svg.area()
                        .interpolate("basis")
                        .x(function (d) {
