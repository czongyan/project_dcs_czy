(function () {
    ...
    var timeseries = function (spaced, flag, data, selectDate, typeFlag, enableBrush, enableZoom) {
        ...
        dateShow.innerHTML = '<div class="time-info" style="height=15px;text-align:center"><i class="time-i">时序图显示时间范围为：' + selectDate[0] + " 至 " + selectDate[1] + '</i></div>';
        //			<button class="clear-brush">Clear brush</button>
        divID.appendChild(dateShow);
        var property_1_0 = [["发电功率", "MW"], ["功率负荷", "MWD2"], ["主蒸汽压力", "MSP"], ["主蒸汽温度", "THRTEMP"]];
        ...
        switch (typeFlag) {
            case 0:
                if (flag == 1) {
                    ...
                    render(divID, spaced, data, property_show, property_data, enableBrush, enableZoom);
                ...
                break;
            case 1:
                brush_obj.push(renderPramater(spaced, data, enableBrush, enableZoom));
                break;
        }
    }; //渲染
    ...
    function render(divID, spaced, dataSet, property_show, property_data, enableBrush, enableZoom) {
        ...
        var x = d3.time.scale().domain(xExtent).range([0, timeWidth]);
        var y = d3.scale.linear().domain(yExtent1).range([timeHeight, 0]);
        var y1 = d3.scale.linear().domain(yExtent2).range([timeHeight, 0]);
        var y2 = d3.scale.linear().domain(yExtent3).range([timeHeight, 0]);
        var y3 = d3.scale.linear().domain(yExtent4).range([timeHeight, 0]);
        //定制x,y轴画布位置
        ...
        //画线
        //定义线性
        var or_line = d3.svg.line().x(function (d) {
            return x(d.date);
        }).y(function (d) {
            return y(d[property_data[0]]);
        });
        var or_line1 = d3.svg.line().x(function (d) {
            return x(d.date);
        }).y(function (d) {
            return y1(d[property_data[1]]);
        });
        var or_line2 = d3.svg.line().x(function (d) {
            return x(d.date);
        }).y(function (d) {
            return y2(d[property_data[2]]);
        });
        var or_line3 = d3.svg.line().x(function (d) {
            return x(d.date);
        }).y(function (d) {
        ...
        //确定选中的时段内分割模块的时段
        var defined_date = [];
        var date_d = dataSet[0].date.getDate();
//        console.log(date_d);
        for (var i in seg_nest) {
            if (seg_nest[i].key == date_d) {
                for (var j in seg_nest[i].values) {
                    var val = seg_nest[i].values[j];
                    if (parseDate.parse(selectDate[0])-val.startDate<=0&& parseDate.parse(selectDate[1])- val.endDate>=0)
                        defined_date.push(val);
                }
            }
        }
//        console.log(defined_date);
        //重定义画线
        temp_1 = dataSet;
        temp_2 = defined_date;
        var line = d3.svg.line().x(function (d) {
            ...
            for (var t in defined_date) {
                var val = defined_date[t];
                if (d.date - val.startDate >= 0 && val.endDate - d.date >= 0) {
                    console.log(t+'存在');
                    return d;
                } 
//                else
//                    return false;
        ...
        //变灰设置
//        var path = svg.append('path').datum(dataSet).attr('class', 'line').attr('d', or_line).style("stroke", function (d) {
//            return "#ccc";
//        });
//
//        var path = svg.append('path').datum(dataSet).attr('class', 'line1').attr('d', or_line1).style("stroke", function (d) {
//            return  "#ccc";
//        });
//        var path = svg.append('path').datum(dataSet).attr('class', 'line2').attr('d', or_line2).style("stroke", function (d) {
//            return  "#ccc";
//        });
//        var path = svg.append('path').datum(dataSet).attr('class', 'line3').attr('d', or_line3).style("stroke", function (d) {
//            return  "#ccc";
//        });
        ...
        var path = svg.append('path').datum(dataSet).attr('class', 'line3').attr('d', line3).style("stroke", function (d) {
            console.log(line_color(property_data[3]));
            return line_color(property_data[3]);
        }).style("stroke-width",2);
        ...
        if (enableBrush) {
            ...
            var brush = d3.svg.brush().x(x).on("brush", brushed).on("brushend", brushend);
            //var zoom = d3.zoom().on("zoom", zoomed);
            //定义画布
            gBrush = svg.append("g").attr("class", "brush").call(brush).selectAll("rect").attr("y",  -6).attr("height",  timeHeight + 7); 
            ...
            function brushend() {
                ...
                cord_f = convertDate(dm[0]), cord_s = convertDate(dm[1]);
                create_div_layoutChange();
                //                draw_auto_coordinate(convertDate(dm[0]), convertDate(dm[1]));
            }
        };
        //		if (enableZoom) {
        //			var zoom = d3.zoom().on("zoom", zoomed);
    ...
    /***detail显示模式模块开始***/
    function create_div_layoutChange() {
        //创建更改菜单
        var layout_change_obj = document.getElementById('layout_change');
        layout_change_obj.innerHTML = "<p>选择布局&nbsp;&nbsp;<select id='data_select' name='data_select' onchange='data_select()'><option value=''>请选择数据类型</option><option value='1'>详细数据</option><option value='2'>统计数据</option></select> <select id='layout_select' name='layout_select' onchange='show_layout()'><option value='' selected='selected'>请选择布局</option><option value='1'>平行坐标</option><option value='2'>散点矩阵</option></select></p>";
        //        var value_obj=d3.select("#data_select").on("change",show_data(this));
        //        var layout_obj=d3.select("#layout_select").on("change",show_layout(f,s));
    }
    ...
    function brush_line_move(d, m) {
        ...
        for (var i in line_brush_selection) {
            if (line_brush_selection[i][0] != d && line_brush_selection[i][1] == false) {
                d3.select("div#" + d + " .brush").call(brush.clamp([m[0], m[1]]));
            }
        }
    }
