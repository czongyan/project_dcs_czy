...
def showCluster(dataSet, k, centroids, clusterAssment):
    numSamples, dim = dataSet.shape  # 数据大小，维度
    # if dim != 2:
    ...
    for i in range(numSamples):
        ...
        if dim != 2:
            plt.plot(i, dataSet[i, 0], mark[markIndex])
        else:
    ...
    for i in range(k):
        # 画图：聚类聚点  点大小12，
        if dim != 2:
            plt.plot(i, centroids[i, 0], mark[i], markersize=12)
        else:
