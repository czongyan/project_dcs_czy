    ...
    function display_seg(data) {
        ...
        for (var i in data) {
            ...
            if (settings.segment) {
                ...
                if (flag > 0) {//满足稳定工况
                    if (d.endIndex == 1 && d.resolution == getSegFlag(d)) {//满足MCR+稳定性
                        var rect = g.append("rect").attr("class", "map_rect").attr("id", "rect-" + i).attr("x", (startData.x))
                            .attr("y", (startData.y-2)).attr("width", (d.dx * (i - startIndex + 1))).attr("height", settings.strokeWidth).style("fill", function () {
                                //方案一：使用原色调(100跨度)
                            ...
                            }).style("stroke-width", 1).style("stroke", "blue").style("stroke-opacity",1).style("opacity", function () {
                                return settings.alpha;
                            });
