...
(function (global) {
    ...
    function display_axis_x() {
        ...
        var xAxis = d3.svg.axis()
            .scale(x).orient("top").ticks(d3.time.minutes, 30).tickFormat(d3.time.format("%H:%M"));
    ...
    function line_fineGrit(data) {
        ...
        for (var i in settings.parameterName) {
            ...
            var line = d3.svg.area()
                ...
                .y(function (d) {
//                    console.log((d[settings.parameterName[i]] - y_domain[0]) / (y_domain[1] - y_domain[0]),y((d[settings.parameterName[i]] - y_domain[0]) / (y_domain[1] - y_domain[0])));
//                    return parseInt(y((d[settings.parameterName[i]] - y_domain[0]) / (y_domain[1] - y_domain[0])*100));
                    return y(d[settings.parameterName[i]]);
                });
//            y.domain([0, 100]);
  y.domain(allDomain[i]);
        ...
        line_g.selectAll(".line_text").data(settings.parameterName).enter().append("line").attr("class", "line_text").attr("x1",function (d, i) {return i * 50;}).attr("y1", 0).attr("x2", function (d, i) {return  i*50+30;}).attr("y2", 0).attr("stroke", function (d, i) {
            return temp_color(i);
        }).attr("stroke-width", "2");
        line_g.selectAll(".text_des").data(settings.parameterName).enter().append("text").attr("class","text_des").attr("x", function (d, i) {return  i*50+30;}).attr("y",0).attr("fill","#000").text(function(d){return d;});
    }
    ...
    function rect_fineGrit(presentIndex, presentID, data) {
        ...
        for (var i in settings.parameterName) {
            ...
            var tempColor_v2 = d3.scale.quantize()
    ...
    function rect_rudeGrit(data) {
        ...
        settings.diffY += settings.strokeWidth;
        //添加日期说明
