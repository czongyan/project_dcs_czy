    ...
    function initLayout(data, i) {
        ...
        if (settings.segment) { //已经分割
            ...
            for (var j = 0; j < len; j++) {
                ...
                data[j].x = j * rect_w;
                data[j].y = (i + settings.gY) * (rect_h + 3);
                data[j].dx = rect_w;
                data[j].dy = rect_h;
    ...
    function display_seg(data) {
        ...
        for (var i in data) {
            ...
            if (settings.segment) { //是，显示三层结构 （RAW--MCR--SEG）
                if (d.resolution > 0 && (settings.nom == 5 ? d.NOM >= 5 : d.NOM == settings.nom) && (d.MW >= settings.mwDomain[0]) && (d.MW < settings.mwDomain[1])) {
                    //绘制MCR满的的第二层工况信息 MCR
                    ...
                    if (flag > 0) { //开始判断是否到稳定工况段结束位置
                        //console.log(d.dateStr,(formatDate(segDate.end) - d.date))
                        //判断是不是衔接上一个稳定工况
                        if (endFlag == -1) {
                            startIndex = d.index;
                            startData = d;
                        ...
                        if ((formatDate(segDate.end) - d.date) >= 0 && (formatDate(segDate.end) - d.date) < 1000000) { //满足时间段
                            flag = 0;
                            //                            console.log(startData, startIndex, d);
                            //*****任务1******画稳定工况（可变色的长方形rect 加边框）
                            var rect = g.append("rect").attr("class", "map-rect").attr("id", "rect-" + i).attr("x", (startData.x))
                                ...
                                .on("mousemove", function () { //focus这个rect时 边框变粗
                                    ...
                                    d3.selectAll('.translated').attr("stroke", "blue").attr("stroke-width", 1);
                                })
                                //添加弹出框
                                .on("click", function () {
                                    ...
                                    detailSvg.append("g").selectAll("rect").data(tempData).enter().append("rect").attr("x", function (di, i) {
                                        return i * 10;
                            ...
                            //1、判断是否结尾
                            if (d.date.getHours() == 24 && d.date.getMinutes() == 50) {
                                ...
                                var rect = g.append("rect").attr("class", "map-rect").attr("id", "rect-" + i).attr("x", (startData.x))
                                    ...
                                    .attr("stroke-width", 1).attr("stroke", "blue").attr("opacity", function () {
                                        ...
                                        d3.selectAll('.translated').attr("stroke-width", 1);
                                    }) //添加弹出框
                                    .on("click", function () {
                                        var index = this.id.split('-')[1];
                                        document.getElementById('light').style.display = 'block';
